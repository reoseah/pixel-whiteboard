...
export const CanvasComponent = (props: {
  ...
  const rerenderChunks = (positions: Map<number, Set<number>>) => {
    ...
    actions.forEach((action) => {
      const type = props.app.resources.actions[action.type]

      positions.forEach((rows, column) => {
        rows.forEach((row) => {
          if (affectsChunk(props.app, action, column, row)) {
  ...
  const onDataChange = (event: Y.YEvent<Y.Array<CanvasActionData>>) => {
    ...
    const deletionAffectedChunks = getChunksAffectedByDeletions(props.app, event)
...
export const affectsChunk = (app: Application, action: CanvasActionData, column: number, row: number) => {
  const type = app.resources.actions[action.type]
...
export const getAffectedChunks = (app: Application, action: CanvasActionData) => {
  const type = app.resources.actions[action.type]
...
const getChunksAffectedByDeletions = (app: Application, event: Y.YEvent<Y.Array<CanvasActionData>>): Map<number, Set<number>> => {
  const chunks = new Map<number, Set<number>>()
  event.changes.deleted.forEach((item) => {
    item.content.getContent().forEach((action) => {
      getAffectedChunks(app, action).forEach((chunk) => {
